...
class GConvFocus(nn.Module):
    ...
    def __init__(self, orin_Focus: nn.Module):
        ...
        self.weight1 = torch.tensor(
            [[1., 0], [0, 0]]).expand(3, 1, 2, 2).to(device)
        self.weight2 = torch.tensor(
            [[0, 0], [1., 0]]).expand(3, 1, 2, 2).to(device)
        self.weight3 = torch.tensor(
            [[0, 1.], [0, 0]]).expand(3, 1, 2, 2).to(device)
        self.weight4 = torch.tensor(
            [[0, 0], [0, 1.]]).expand(3, 1, 2, 2).to(device)
        self.__dict__.update(orin_Focus.__dict__)
